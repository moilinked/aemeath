...
// GetForUser / ListForUser / UpdateTitleForUser / ClearMessagesForUser / DeleteForUser
// 只暴露当前用户拥有的对话；不存在或不属于该用户时返回 ErrConversationNotFound。
type ConversationStore interface {
	...
	ClearMessagesForUser(ctx context.Context, userID string, conversationID string) (Conversation, error)
